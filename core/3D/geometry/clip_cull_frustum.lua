...
return function(object,tri_list,a,b,c,n,fs,index)
    ...
    if v1z < 0 then
        if v2z < 0 then
            n = n + 1
            clip_2(object,n,tri_list,a,b,c,fs,index)
        elseif v3z < 0 then
            n = n + 1
            clip_2(object,n,tri_list,a,c,b,fs,index)
        else
            n = n + 2
            clip_1(object,n,tri_list,a,b,c,fs,index)
        end
    elseif v2z < 0 then
        if v3z < 0 then
            n = n + 1
            clip_2(object,n,tri_list,b,c,a,fs,index)
        else
            n = n + 2
            clip_1(object,n,tri_list,b,a,c,fs,index)
        end
    elseif v3z < 0 then
        n = n + 2
        clip_1(object,n,tri_list,c,a,b,fs,index)
    else
        n = n + 1
        tri_list[n] = {a,b,c,fs=fs,object=object,index=index}
    end
    return n
end